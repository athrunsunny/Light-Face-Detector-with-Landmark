...
class Facenano(nn.Module):
    def __init__(self):
        super(Facenano, self).__init__()
        self.num_classes = 2  # 1、bg 2、face
        ...
        self.conv7 = DWConvblock(64, 96, 2)
        self.conv8 = DWConvblock(96, 96, 1)

        self.conv9 = DWConvblock(96, 128, 2)
        self.conv10 = DWConvblock(128, 128, 1)

        self.conv11 = nn.Sequential(
            nn.Conv2d(in_channels=128, out_channels=64, kernel_size=1),
            nn.ReLU(inplace=True),
            DPConv(64, 128, kernel=3, stride=2, pad=1),
            nn.ReLU(inplace=True)
        )
    ...
    def multibox(self, num_classes):
        ...
        loc_layers += [DPConv(96, 2 * 4, kernel=3, pad=1)]
        conf_layers += [DPConv(96, 2 * num_classes, kernel=3, pad=1)]
        lmk_layers += [DPConv(96, 2 * 10, kernel=3, pad=1)]
        ...
        loc_layers += [nn.Conv2d(128, 3 * 4, kernel_size=3, padding=1)]
        conf_layers += [nn.Conv2d(128, 3 * num_classes, kernel_size=3, padding=1)]
        lmk_layers += [nn.Conv2d(128, 3 * 10, kernel_size=3, padding=1)]
    ...
    def forward(self, inputs):
        ...
        for (x, l, c, lm) in zip(layers, self.loc, self.conf, self.lmk):
...
def create_net(model='Facenano', input=None, print_model=False):
    net = eval(model)()
    if input is None:
...
if __name__ == '__main__':
    create_net(model='Facenano', input=torch.randn(1, 3, 600, 600), print_model=True)
