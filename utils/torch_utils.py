...
def smart_optimizer(model, name='Adam', lr=0.001, momentum=0.9, decay=1e-5, general_optim=False):
    if general_optim:
        if name == 'Adam':
        ...
        LOGGER.info(f"{colorstr('optimizer:')} {type(optimizer).__name__}(lr={lr}) with general optimizer ")
    else:
